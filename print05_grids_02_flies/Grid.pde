class ModularGrid
{
  ...
  ModularGrid(int _cols, int _rows, float _gutterSize, float _pageMarginX, float _pageMarginY, int _width, int _height)
  {
    width = _width;
    height = _height;
    cols = _cols;
    ...
    pageMarginX = _pageMarginX;
    pageMarginY = _pageMarginY;
    ...
    float actualPageWidth = ((float)width - (2*pageMarginX));
    ...
    float actualPageHeight = ((float)height - (2*pageMarginY));
    ...
    for(int i = 0; i < cols; i++)
    {
      for(int j = 0; j < rows; j++)
      {
        ...
        modules[i][j].x = pageMarginX + (i*moduleWidth) + (i*gutterSize);
        modules[i][j].y = pageMarginY + (j*moduleHeight) + (j*gutterSize);
  ...
  Module getModule( int x, int y ){
    return modules[x][y];
  }
  
  Module getRandomModule(int numModulesHorizontal, int numModulesVertical, boolean canOverlap)
  {
    ...
    for(int i = 0; i < cols; i++)
    {
      for(int j = 0; j < rows; j++)
      {
        if(!modules[i][j].used || canOverlap)
        {
          int[] index = {i, j};
    ...
    {
      if(modules[selectedCol + i][selectedRow].used && !canOverlap)
      {
    ...
    for(int i = 0; i < numModulesVertical; i++)
    {
      ...
      {
        if(modules[selectedCol + j][selectedRow + i].used && !canOverlap)
        {
  ...
  void display( color c )
  {
    ...
    canvas.rect(pageMarginX, pageMarginY, width - (2*pageMarginX), height - (2*pageMarginY));
